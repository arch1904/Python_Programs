...
def basic_check(n):
    ...
    (since we're not allowed to use a dictionary)
    '''
    if n==0:
        return "zero"
    elif n==1:
...
def number_to_text(number):
    ...
    '''
    if basic_check(number) is not None: #if number is already a basic number
        return basic_check(number)
    elif number>=0 and number<=100:
        return basic_check(number-number%10) + ' ' + basic_check(number%10) #get units and tens digit seperately and concatenate
    else:
        return "OUT OF BOUNDS!" #if number is not (0-100)
